...
int main(int argc, char *argv[]) {
    ...
    for (; argnum < argc && strncmp(argv[argnum], "--", 2) == 0;) {
        ...
        if (arg == "--annotate") {
            annotate = true;
            if (argnum < argc) {
                string outputFile(argv[argnum]);
                argnum++;
                annotateOutput = outputFile;
            } else {
...
void sendGame(vector<string> &movelist, const string& fenstring, int bookDepth) {
    ...
    if (okToAnalyse) {
                    ...
                    if (playedMoveEvaluated) {
                        ...
                        if (it != evaluations.end()) {
                            ...
                            if (ev->isForcedMate()) {
                                ...
                                outdata << "mate " << ev->getNumMateMoves();
                            } else {
                                cout << ev->getValue();
                                outdata << ev->getValue();
                            }
        ...
        if (annotate) {
            ...
            outdata << movelist[movelist.size() - 1] << endl;
            if (XMLformat) {
                cout << "</annotation>" << endl;
            }
        } else {
